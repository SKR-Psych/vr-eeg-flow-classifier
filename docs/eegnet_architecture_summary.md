...
* **Tailored for Low-SNR Neuroimaging:** Standard 2D computer-vision CNNs overfit rapidly on multichannel EEG due to parameter explosion and temporal-spatial correlation mixing. EEGNet uses specialised, factorised convolutions that decouple **temporal frequency learning** from **spatial electrode topography**, requiring only **~2,500 trainable parameters**.
* **Direct Raw Microvolt Ingestion:** Eliminates the need for manual feature engineering (e.g. Welch PSD, Hilbert transforms, or phase-locking values), ingesting normalised 4D tensors directly:
...
* **Temporal Convolution ($1 \times 64$ kernel, $F_1 = 8$ filters):**
  * Acts as a learnt digital filterbank across time without channel mixing.
...
* **Depthwise Spatial Convolution ($64 \times 1$ kernel, depth multiplier $D = 2$):**
  * Convolves spatially across all 64 scalp electrodes independently for each learnt temporal filter.
  * Mathematically equivalent to learning **spatial filters** (analogous to Common Spatial Patterns [CSP] or spatial beamforming) to isolate localised cortical dipole sources.
...
* **Depthwise Temporal ($1 \times 16$) + Pointwise ($1 \times 1$, $F_2 = 16$ filters):**
  * Decoupled architecture first summarises individual feature maps over an extended temporal window, then mixes representations across feature maps.
...
* **ELU Activation & Average Pooling ($1 \times 8$):**
  * Exponential Linear Units preserve negative signal deflections crucial for biphasic Event-Related Potentials (ERPs) whilst preventing dead neurons.
...
1. **Sub-Second Event Locking:**
   * Operates on $700\text{ ms}$ windows centred on the contact event (`box:touched`). This captures the transient ($150\text{–}350\text{ ms}$) **Prediction Error Negativity (PEN/N200)** that was completely diluted by the legacy SVM's wide $2.0\text{s}$ sliding windows.
2. **Phase Topology Preservation:**
   * Whilst legacy feature extractors collapsed 64 channels into 14 scalar averages, EEGNet preserves cross-channel phase relationships and electrode geography.
3. **Real-Time Latency:**
